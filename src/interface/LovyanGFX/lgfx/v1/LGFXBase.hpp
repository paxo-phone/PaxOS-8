...
#define LGFX_PRINTF_ENABLED
  ...
  public:
    ...
    LGFX_INLINE_T void fillRoundRect   ( int32_t x, int32_t y, int32_t w, int32_t h, int32_t r, const T& color) { setColor(color); fillRoundRect(x, y, w, h, r); }
                  ...
                  void drawRoundRectWithBorder( int32_t x, int32_t y, int32_t w, int32_t h, int32_t r, int32_t bs, uint16_t Backcolor, uint16_t Bordercolor )
                  {
                    if(bs != 0)
                      fillSmoothRoundRect(x, y, w, h, r, Bordercolor);
                    fillSmoothRoundRect(x+bs, y+bs, w-2*bs, h-2*bs, r-bs, Backcolor);
                  }
